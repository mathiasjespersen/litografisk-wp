...
<section class="artist-items" data-title="<?= $term->name; ?>">
	<nav class="subnav">
		<a href="/shop" class="arrow left">Tilbage til shop</a>
	</nav>
		...
		<article data-timestamp="<?= get_post_time('U', true); ?>" data-id="<?= $term->slug; ?>">
			<div class="span4">
				...
				if (get_the_post_thumbnail()) :
					...
					$image_title = get_the_title() . ' ' . $term->name . ' ' . wp_strip_all_tags( get_the_content()); 
					?>
			...
			<div class="span2">			
				<h2><?php the_title(); ?></h2>
				<div class="artist"><?= $term->name; ?></div>
				<div class="info">
					<?php the_content(); ?>
				</div>
				<div class="buy">
					<a href="#" class="buy-btn">Køb</a>
				</div>
			</div>
		</article>
